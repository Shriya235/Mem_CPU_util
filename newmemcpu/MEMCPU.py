...
def mem(sub,logfile):                       # subroutine for memory utilisation
    mem = []
    newlistmem = []
    memtime=[]
    tim=[]
    ...
    for (value,total) in zip(mem[2::10],mem[1::10]):
        ...
        newlistmem.append(new_elem*100)             #newlistmem has the final values
...
def cpu(str1,logfile):                          #subroutine for cpu utilisation
    cpu=[]
    newlistcpu=[]
    cputime=[]
    substr1= str1
    ...
    for value in cpu[7::11]:
        newlistcpu.append(100-float(value))         #newlistcpu has the final values
...
def Data_Printing(value,string1):                  #prints the max,min and avg of the values
    if string1 == "Mem:      ":
...
def Data_Plotting(x,y,title):                       #plots the mem and cpu utilisation over time
    ax=plt.gca()
...
def Data_preparation(fname,avg,memory):             #calls for the required memory or cpu utilisation methods
    mem(memory,fname)    
    for i in avg:
        cpu(i,fname)
